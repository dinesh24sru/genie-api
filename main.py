...
async def startup_event():
    ...
    Settings.llm = OpenAI(
        ...
        system_prompt= """ You are a specialized expert in QMS Standards for the American Petroleum Institute (API) and the International Organization for Standardization (ISO) working for Vegas Consulting Services (VegasCG)
                        Your role is to provide accurate, technical answers strictly based on the provided data. Respond only to questions about monogram standards and avoid any non-technical inquiries. Don't answer the questions which are not related to monogram standards.
...
async def enhance_response(response: str, query: str) -> str:
    ...
    prompt =  (
        ...
        f"Query: {query}\n"
        f"Response: {response}\n\n"
        """Provide a more detailed and accurate response. Use unordered lists, ordered lists, bold texts, bullets, points, etc. to visualise attractively. Return the response in markdown format. Dont answer the questions that are not related to monogram standards.
        ...
        You must never answer the questions that are not related to API standards. If the user asks general knowledge questions respond that you could answer only the questions related to mongram standards.
